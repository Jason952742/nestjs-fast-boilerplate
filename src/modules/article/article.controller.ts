...
export class ArticleController {
  ...
  @ApiResponse({ status: 403, description: 'Forbidden.' })
  @Put(':slug')
  async update(@User('id') user: number, @Param() params: any, @Body('article') articleData: CreateArticleDto) {
  ...
  async deleteComment(@User('id') user: number, @Param() params: any) {
    const { slug, id } = params
    return this.articleService.deleteComment(+user, slug, +id)
  }

  @ApiOperation({ summary: 'Favorite article' })
  @ApiResponse({ status: 201, description: 'The article has been successfully favorite.' })
  @ApiResponse({ status: 403, description: 'Forbidden.' })
  @Post(':slug/favorite')
  ...
  @ApiOperation({ summary: 'Un favorite article' })
  @ApiResponse({ status: 201, description: 'The article has been successfully un favorite.' })
  @ApiResponse({ status: 403, description: 'Forbidden.' })
